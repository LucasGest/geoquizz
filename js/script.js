...
function matchCountry(input, c, field = "name") {
  ...
  if (field === "name") {
    ...
    return c.capital && fuzzy(input, c.capital);
  }
}
...
function renderQ() {
  ...
  if (selMode === "flag2country") {
    ...
    renderMCQ(getOpts(c, 4), c, (o) => o.capital);
...
function submitTxt() {
  ...
  if (ok) {
    ...
    showFb(
      false,
      `✗ C'était : ${selMode === "write-country" ? c.nameFr || c.name : c.capital}`,
    );
  }
  updateStats();
  document.getElementById("nextBtn").className = "next-btn show";
...
function mapSubmit() {
  ...
  setMapFb(
    `✅ ${match.nameFr || match.name} — capitale : ${match.capital}`,
    "ok",
  );
